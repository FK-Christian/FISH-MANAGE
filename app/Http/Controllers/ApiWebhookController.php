    ...
    private function evaluateInput() {
        ...
        if (strcmp(trim(strtoupper($value)), "RESET") == 0) {
            ...
            if (strcasecmp($this->navigation->customer_current_step, $this->navigation->customer_next_step) != 0) {
                ...
                $this->update_where("chat_id = '" . $this->navigation->customer_chat_id . "'", 'navigation', array('last_date' => date('Y-m-d H:i:s'), 'step' => $this->navigation->customer_next_step));
            }
        }
    }
    ...
    private function getNextPosition() {
        ...
        switch ($step) {
            ...
            case "NOTFOUND":
                ...
                break;
            case "BAC":
                $lesBacs = Bac::all()->orderBy('id','desc')->get(); //$this->get_data_by("*", "bacs", "1=1", '', '', 'id asc');
                ...
                break;
            case "VAGUE":
                $lesVagues = Vague::all()->orderBy('id','desc')->get();
                ...
                break;
            case "BAC2":
                $lesBacs = Bac::all()->orderBy('id','desc')->get();
                ...
                break;
            case "ALIMENT":
                $lesAliments = Aliment::all()->orderBy('id','desc')->get();
